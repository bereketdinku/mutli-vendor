...
const conditionalDevServer = (fileKey, apiKey)=>{
    return Effect__namespace.gen(function*() {
        const file = yield* shared.fetchEffJson(shared.generateUploadThingURL(`/api/pollUpload/${fileKey}`), PollUploadResponseSchema).pipe(Effect__namespace.andThen((res)=>res.status === "done" ? Effect__namespace.succeed(res.fileData) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
...
const completeMultipartUpload = (presigned, etags)=>shared.fetchEffJson(shared.generateUploadThingURL("/api/completeMultipart"), S__namespace.Struct({
        success: S__namespace.Boolean,
        message: S__namespace.optional(S__namespace.String)
    }), {
        method: "POST",
...
const abortMultipartUpload = (presigned)=>shared.fetchEffJson(shared.generateUploadThingURL("/api/failureCallback"), FailureCallbackResponseSchema, {
        method: "POST",
        body: JSON.stringify({
            fileKey: presigned.key,
            uploadId: presigned.uploadId
...
const handleCallbackRequest = Effect__namespace.gen(function*() {
    ...
    const requestInput = yield* shared.parseRequestJson(req, S__namespace.Struct({
        status: S__namespace.String,
    ...
    yield* shared.fetchEffJson(shared.generateUploadThingURL("/api/serverCallback"), ServerCallbackPostResponseSchema, {
        method: "POST",
...
const handleUploadAction = Effect__namespace.gen(function*() {
    ...
    const { files, input } = yield* shared.parseRequestJson(opts.req, UploadActionPayload);
    ...
    const presignedUrls = yield* shared.fetchEffJson(shared.generateUploadThingURL("/api/prepareUpload"), PresignedURLResponseSchema, {
        method: "POST",
        body: JSON.stringify({
...
const handleMultipartCompleteAction = Effect__namespace.gen(function*() {
    const opts = yield* RequestInput;
    const requestInput = yield* shared.parseRequestJson(opts.req, MultipartCompleteActionPayload);
...
const handleMultipartFailureAction = Effect__namespace.gen(function*() {
    ...
    const { fileKey, uploadId } = yield* shared.parseRequestJson(req, FailureActionPayload);
...
const uploadPresignedPost = (file, presigned)=>Effect__namespace.gen(function*() {
        ...
        }).pipe(Effect__namespace.tapErrorCause(()=>shared.fetchEffJson(shared.generateUploadThingURL("/api/failureCallback"), FailureCallbackResponseSchema, {
                method: "POST",
                body: JSON.stringify({
                    fileKey: presigned.key,
                    uploadId: null
...
const getPresignedUrls = (input)=>Effect__namespace.gen(function*() {
        ...
        const presigneds = yield* shared.fetchEffJson(shared.generateUploadThingURL("/api/uploadFiles"), responseSchema, {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                files: fileData,
        ...
        }).pipe(Effect__namespace.catchTag("ParseError", (e)=>Effect__namespace.die(e)), Effect__namespace.catchTag("FetchError", (e)=>Effect__namespace.die(e)));
...
const uploadFile = (input)=>Effect__namespace.gen(function*() {
        ...
        yield* shared.fetchEffJson(shared.generateUploadThingURL(`/api/pollUpload/${presigned.key}`), PollUploadResponseSchema).pipe(Effect__namespace.tap(Effect__namespace.logDebug("Polled upload", presigned.key)), Effect__namespace.andThen((res)=>res.status === "done" ? Effect__namespace.succeed(undefined) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
            while: (err)=>err instanceof shared.RetryError,
            schedule: shared.exponentialBackoff
        }), Effect__namespace.catchTag("RetryError", (e)=>Effect__namespace.die(e)));
        return {
...
class UTApi {
    constructor(opts){
        this.requestUploadThing = (pathname, body, responseSchema)=>{
            ...
            return shared.fetchEffJson(url, responseSchema, {
                method: "POST",
                cache: "no-store",
