import { ContentDispositionSchema, fetchEffJson, generateUploadThingURL, RetryError, exponentialBackoff, UploadThingError, signPayload, fetchEff, isObject, contentDisposition, bytesToFileSize, getTypeFromFileName, objectKeys, InvalidRouteConfigError, fileSizeToBytes, FetchContext, getFullApiUrl, verifySignature, parseRequestJson, fillInputRouteConfig, filterObjectValues, asArray, getStatusCodeFromError } from '@uploadthing/shared';
...
const conditionalDevServer = (fileKey, apiKey)=>{
    return Effect.gen(function*() {
        const file = yield* fetchEffJson(generateUploadThingURL(`/api/pollUpload/${fileKey}`), PollUploadResponseSchema).pipe(Effect.andThen((res)=>res.status === "done" ? Effect.succeed(res.fileData) : Effect.fail(new RetryError())), Effect.retry({
...
const completeMultipartUpload = (presigned, etags)=>fetchEffJson(generateUploadThingURL("/api/completeMultipart"), S.Struct({
        success: S.Boolean,
        message: S.optional(S.String)
    }), {
        method: "POST",
...
const abortMultipartUpload = (presigned)=>fetchEffJson(generateUploadThingURL("/api/failureCallback"), FailureCallbackResponseSchema, {
        method: "POST",
        body: JSON.stringify({
            fileKey: presigned.key,
            uploadId: presigned.uploadId
...
const handleCallbackRequest = Effect.gen(function*() {
    ...
    const requestInput = yield* parseRequestJson(req, S.Struct({
        status: S.String,
    ...
    yield* fetchEffJson(generateUploadThingURL("/api/serverCallback"), ServerCallbackPostResponseSchema, {
        method: "POST",
...
const handleUploadAction = Effect.gen(function*() {
    ...
    const { files, input } = yield* parseRequestJson(opts.req, UploadActionPayload);
    ...
    const presignedUrls = yield* fetchEffJson(generateUploadThingURL("/api/prepareUpload"), PresignedURLResponseSchema, {
        method: "POST",
        body: JSON.stringify({
...
const handleMultipartCompleteAction = Effect.gen(function*() {
    const opts = yield* RequestInput;
    const requestInput = yield* parseRequestJson(opts.req, MultipartCompleteActionPayload);
...
const handleMultipartFailureAction = Effect.gen(function*() {
    ...
    const { fileKey, uploadId } = yield* parseRequestJson(req, FailureActionPayload);
...
const uploadPresignedPost = (file, presigned)=>Effect.gen(function*() {
        ...
        }).pipe(Effect.tapErrorCause(()=>fetchEffJson(generateUploadThingURL("/api/failureCallback"), FailureCallbackResponseSchema, {
                method: "POST",
                body: JSON.stringify({
                    fileKey: presigned.key,
                    uploadId: null
...
const getPresignedUrls = (input)=>Effect.gen(function*() {
        ...
        const presigneds = yield* fetchEffJson(generateUploadThingURL("/api/uploadFiles"), responseSchema, {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                files: fileData,
        ...
        }).pipe(Effect.catchTag("ParseError", (e)=>Effect.die(e)), Effect.catchTag("FetchError", (e)=>Effect.die(e)));
...
const uploadFile = (input)=>Effect.gen(function*() {
        ...
        yield* fetchEffJson(generateUploadThingURL(`/api/pollUpload/${presigned.key}`), PollUploadResponseSchema).pipe(Effect.tap(Effect.logDebug("Polled upload", presigned.key)), Effect.andThen((res)=>res.status === "done" ? Effect.succeed(undefined) : Effect.fail(new RetryError())), Effect.retry({
            while: (err)=>err instanceof RetryError,
            schedule: exponentialBackoff
        }), Effect.catchTag("RetryError", (e)=>Effect.die(e)));
        return {
...
class UTApi {
    constructor(opts){
        this.requestUploadThing = (pathname, body, responseSchema)=>{
            ...
            }).pipe(Effect.catchTag("FetchError", (err)=>Effect.logError("Request failed:", err).pipe(Effect.andThen(()=>Effect.die(err)))), Effect.catchTag("ParseError", (err)=>Effect.logError("Response parsing failed:", err).pipe(Effect.andThen(()=>Effect.die(err)))), Effect.tap((res)=>Effect.logDebug("UploadThing response:", res)));
        };
